...
export default function ProfilePage() {
    // Replace with your real auth context / API fetch
    const currentUser = {
        name: "Darius Paulauskas",
        email: "darius@stud.ktu.lt",
    }
    ...
    const profileForm = useForm({
        ...
        onSubmit: async ({ value }) => {
            ...
            try {
                ...
                // await fetch("/api/profile", { method: "PUT", body: JSON.stringify(result.output) })
                        ...
                        <FieldSet>
                            ...
                            <FieldGroup>
                                ...
                                <profileForm.Field name="name">
                                    {(field) => {
                                        const result = v.safeParse(ProfileSchema.entries.name, field.state.value)
                                        const error = !result.success && field.state.meta.isDirty
                                ...
                                <profileForm.Field name="email">
                                    {(field) => {
                                        const result = v.safeParse(ProfileSchema.entries.email, field.state.value)
                                        const error = !result.success && field.state.meta.isDirty
                                            ? result.issues[0].message
                                            : undefined
                                        return (
